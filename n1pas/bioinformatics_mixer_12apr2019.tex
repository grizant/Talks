...
\begin{itemize}
\item We'd like a methodology to estimate biological dysregulation using paired samples from cancer patients.
...
\item Moreover, the setting invovles large-scale simultaenous signification under correlated test statistics.
\end{itemize}
\end{frame}

\begin{frame}\frametitle{This work will appear in Frontiers in Genetics}
\begin{figure}[htb]
  \centering \includegraphics[keepaspectratio,width=\textwidth,height=0.8\textheight]{figures/frontiers.png}
\end{figure}
\end{frame}
...
%%    \begin{align*}
%%      p_{0} & = Pr\{null\}, & f_{0}(z) density if \; null \\
%%      p_{1} & = Pr\{nonnull\}, &  f_{1}(z), density if \; nonnull
%%    \end{align*}
...
%%   We follow Johnson \& Purdom\addtocounter{footnote}{-1}\footnotemark and use of the Hellinger distance to quantify alternative splicing between a pair of samples. Let the estimates of isoform expression for a sample $A$ be denoted as $x_{gA1},\ldots,x_{gAK_{g}}$ for the $K_{g}$ distinct isofrms annotated to gene $g$. We define the relative isoform usage as the vector of relative proportions of each isoform,
...
\begin{frame}\frametitle{Summary of contributions}
  ...
  \item We developed the N-of-1-pathways Alternatively Spliced (N1PAS) framework.
    ...
    \item We validated the method using target KEGG pathways (see paper).
    \item And applied N1PAS output to survival subtyping (see paper).
